...
class ResNet(nn.Module):
    ...
    def __init__(self, arch, block, layers, output_stride, BatchNorm, pretrained=True):
        ...
        if self.arch == 'resnet18':
            ...
            self.latlayer1 = nn.Conv2d(256, 128, kernel_size=1, stride=1, padding=0)
            self.latlayer2 = nn.Conv2d(128, 128, kernel_size=1, stride=1, padding=0)
            self.latlayer3 = nn.Conv2d(64, 128, kernel_size=1, stride=1, padding=0)
        else:
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, BatchNorm=None):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, dilation, downsample, BatchNorm, groups=self.groups,
                            base_width=self.base_width))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, dilation=dilation, BatchNorm=BatchNorm, groups=self.groups,
                                base_width=self.base_width))
    ...
    def _make_MG_unit(self, block, planes, blocks, stride=1, dilation=1, BatchNorm=None):
        ...
        for i in range(1, len(blocks)):
            layers.append(block(self.inplanes, planes, stride=1,
                                dilation=blocks[i] * dilation, BatchNorm=BatchNorm, groups=self.groups,
                                base_width=self.base_width))
    ...
    def forward(self, input):
        ...
        # Top-down
        p5 = self.toplayer(c5)
        p4 = F.upsample(p5, size=c4.size()[2:], mode='bilinear') + self.latlayer1(c4)
        p3 = F.upsample(p4, size=c3.size()[2:], mode='bilinear') + self.latlayer2(c3)
        p2 = F.upsample(p3, size=c2.size()[2:], mode='bilinear') + self.latlayer3(c2)
...
if __name__ == "__main__":
    ...
    model = FPN18(BatchNorm=nn.BatchNorm2d, pretrained=True, output_stride=32)
    stat(model.cpu(), input_size=(3, 256, 256))
